...
def freeze_graph(input_checkpoint,output_graph):
    ...
    :return:
    '''
    checkpoint = tf.train.get_checkpoint_state(input_checkpoint+'/../') #检查目录下ckpt文件状态是否可用
    # input_checkpoint = checkpoint.model_checkpoint_path #得ckpt文件路径
 
    # 指定输出的节点名称,该节点名称必须是原模型中存在的节点
    #output_node_names = "ssd_losses/softmax/Softmax"
    output_node_names = "ssd_losses/softmax/Softmax"
...
if __name__ == '__main__':
    # 输入ckpt模型路径
    input_checkpoint='train_model/model.ckpt-4000'
    # 输出pb模型的路径
    out_pb_path="train_model/pb/frozen_model.pb"
    # 调用freeze_graph将ckpt转为pb
    checkpoint_path=os.path.join(input_checkpoint)
